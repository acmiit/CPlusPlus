...
int main(int argc, char **argv)
{
    ...
    // 2、主动发送连接请求，使用connect函数，但是需要知道服务器的IP与端口号port，使用struct sockaddr_in结构体
    ...
    //3、如果connect返回值是ok，那么就表明三次握手建立成功了，那么就可以进行数据的转发，也就是read/recv 或者write/send函数
    while(1){
        char buf[128]={0};

        cout<<"我客户端想发给数据给服务器 "<<endl;
        string line;
