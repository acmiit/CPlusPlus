...
//对于EventLoop/Reactor而言，即使拿到处理好的之后
//的数据msg,也不能讲数据发送给客户端，因为对于
//EventLoop/Reactor而言，它本身没有发送数据的能力
// 
//真正在整个设计中，具备发送能力的类只有两个
//一个是TcpConnection，另外一个是SocketIO，而
//SocketIO更偏向底层，真正对外发送数据的其实就是
//TcpConnection对象，或者更具体的说，就是
//TcpConnection中的send函数
//所有需要将发送数据能力的函数以及要发送的数据一起
//打包成"任务"交给EventLoop/Reactor，这样EventLoop
//Reactor就可以将数据发给客户端
...
InetAddress TcpConnection::getPeerAddr()
{
    ...
    void TcpConnection::handleNewConnectionCallback(){
        if(_onNewConnectCb){
            ...
            cout<<"_onNewConnectCb == nullptr"<<endl;
        }
    }
